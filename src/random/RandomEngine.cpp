    ...
    else // try to use "location_value_nm_s" property and fetch a point based on a list of points provided ad-hock
    {
      ...
      { // read random element
        ...
        if (xPoint.isEmpty ())
        {
          ...
          this->setError (fmt::format ("[{}] Could not randomly find element <point> in {} node.", __func__, xLocationNodePtr.getName ()) );
          return false;
        }
...
bool
RandomEngine::parse_display_object_element (IXMLNode &inFlightLegNode, IXMLNode &inDisplayNode, IXMLNode & xRootTemplate, IXMLNode & x3DObjTemplate, double &expected_slope_at_target_location_d, std::string & inout_err)
{
...
RandomEngine::generateRandomMissionBasedOnContent (IXMLNode &xTemplateNode)
{
  ...
  for (std::string optional; auto tagName : listContent) // loop over list of "tag_names" that we received from splitting the "list=" attribute
  {
    ...
    if (xFlightLegNodeFromTemplate.isEmpty ())
    {
      ...
      this->setError ( fmt::format ("[{}] \"{}\", element was not found. Please fix template. Aborting mission creation.", __func__, tagName ) );
      return false;
    }
...
RandomEngine::buildFlightLeg (int inFlightLegCounter, const IXMLNode &in_legNodeFromTemplate)
{
  ...
  // v25.08.1 support for "location_properties" attribute that will replace "location_value"
  std::string location_value_nm_s = (!Utils::readAttrib (xExpectedLocation, mxconst::get_ATTRIB_LOCATION_PROPERTIES (), "").empty())? Utils::readAttrib (xExpectedLocation, mxconst::get_ATTRIB_LOCATION_PROPERTIES (), "") : Utils::readAttrib (xExpectedLocation, mxconst::get_ATTRIB_LOCATION_VALUE (), "");
  ...
  const auto lmbda_add_all_display_object_xxx_elements = [&] (const IXMLNode &inParentNode, IXMLNode &inoutTargetNode)
  {
    ...
    for (int i1 = 0; i1 < nDisplayObjects; ++i1)
    {
      ...
      if (std::string err; parse_display_object_element (inoutTargetNode, cNode, this->xRootTemplate, this->x3DObjTemplate, this->expected_slope_at_target_location_d, err)) // v3.0.219.1 handle <display_object> options like: optional, random_water or limit_to_terrain_slope
      {
...
NavAidInfo
RandomEngine::get_random_airport_from_db (missionx::Point &inPoint, const float inMinDistance_nm, const float inMaxDistance_nm, const int inExcludeAngle, missionx::mx_base_node &inProperties)
{
  #ifndef RELEASE
...
RandomEngine::get_target (NavAidInfo &outNewNavInfo, const IXMLNode &inLegFromTemplate, const mx_plane_types in_plane_type_enum, std::map<std::string, std::string> &inMapLocationSplitValues, missionx::mx_base_node &inProperties)
{
  ...
  const auto lmbda_get_ramp_for_plane_type = [&] (missionx::NavAidInfo &out_new_nav_info, const float additional_search_radius_f = 10.f)
  {
    ...
    if (!this->filterAndPickRampBasedOnPlaneType (out_new_nav_info, this->errMsg, missionx::mxFilterRampType::airport_ramp)) // v3.303.12_r2 // try to get Navaid information. If we fail to find information, we ignore and continue with original xPoint data
    {
      ...
      #ifdef IBM
      out_new_nav_info = this->get_random_airport_from_db (this->shared_navaid_info.p, this->shared_navaid_info.inMinDistance_nm, this->shared_navaid_info.inMaxDistance_nm, this->shared_navaid_info.inExcludeAngle, inProperties); // v3.0.255.3 test integration
      #else
      const NavAidInfo nav = this->get_random_airport_from_db (this->shared_navaid_info.p, this->shared_navaid_info.inMinDistance_nm, this->shared_navaid_info.inMaxDistance_nm, this->shared_navaid_info.inExcludeAngle, inProperties); // v3.0.255.3 test integration
      outNewNavInfo        = nav;
      #endif
  ...
  {
    #ifdef IBM
    outNewNavInfo = this->get_random_airport_from_db (this->lastFlightLegNavInfo.p, 3.0f, 50.0f, -1, inProperties); // v3.0.255.3 test integration
    #else
    NavAidInfo nav = this->get_random_airport_from_db (this->lastFlightLegNavInfo.p, 3.0f, 50.0f, -1, inProperties); // v3.0.255.3 test integration
    outNewNavInfo  = nav;
    #endif
    if (outNewNavInfo.lat == 0 || outNewNavInfo.lon == 0) // fallback code if DB logic failed
    {
  ...
  else if (this->lastFlightLegNavInfo.lat != 0.0f && this->lastFlightLegNavInfo.lon != 0.0f)
  {
    ...
    else if ((inLocationType == mxconst::get_EXPECTED_LOCATION_TYPE_OSM ()) || (inLocationType == mxconst::get_EXPECTED_LOCATION_TYPE_WEBOSM ()) || (location_value_tag_name_s.empty () && ((location_value_d > 0.0) || (location_minDistance_d > 0.0 && location_maxDistance_d > 0.0)))) // v3.0.255.3 changed last logic
    {
      ...
      if ((inLocationType == mxconst::get_EXPECTED_LOCATION_TYPE_XY () || inLocationType == mxconst::get_EXPECTED_LOCATION_TYPE_OSM () || inLocationType == mxconst::get_EXPECTED_LOCATION_TYPE_WEBOSM ()) && this->template_plane_type_enum == missionx::mx_plane_types::plane_type_helos && !flag_is_last_flight_leg)
      {
...
RandomEngine::prepare_message_with_special_keywords (missionx::NavAidInfo &inNavAid, const std::string &inMessage)
{
  ...
  inNavAid.fpln_distance_between_prev_and_current_navaid = lmbda_get_distance_between_2_nav_points (inNavAid, this->listNavInfo);
...
{
  // prepare default description for <leg>
  ...
  // std::string    err;
  // const IXMLNode xml_custom_desc_from_target_leg_node = Utils::xml_get_node_randomly_by_name_IXMLNode (in_leg_as_navaid.fpln_xml_osm_q_node, mxconst::get_ELEMENT_DESC (), err);

  // get random node copy
  const IXMLNode xml_custom_desc_from_target_leg_node = Utils::xml_get_node_randomly_by_name_IXMLNode (in_leg_as_navaid.fpln_xml_osm_q_node, mxconst::get_ELEMENT_DESC (), false);

  #ifndef RELEASE
  Log::logMsgThread ( fmt::format ( "[{}] <{}> info:\n{}\n<-- end fpln_xml_target_leg_node \n"
                    , __func__, in_leg_as_navaid.fpln_xml_osm_q_node.getName (),  Utils::xml_get_node_content_as_text ( in_leg_as_navaid.fpln_xml_osm_q_node ) ) );
  #endif
...
RandomEngine::gen_leg_node (const std::string &prefix_name, const std::string &postfix_name, missionx::NavAidInfo *inTargetNavAid, std::list<missionx::structs::strct_node_attribute_key_value> *in_attrib_list, IXMLNode *parentNode)
{
  ...
  // copy all subnodes nodes from the <q> if present
  bool isEmpty = inTargetNavAid->fpln_xml_osm_q_node.isEmpty ();

  if (!(inTargetNavAid->fpln_xml_osm_q_node.isEmpty ()))
  {
    ...
    Utils::xml_copy_or_replace_sub_nodes (leg_node, inTargetNavAid->fpln_xml_osm_q_node, true, &in_exclude_nodes);
  }
...
RandomEngine::gen_briefer_node (missionx::Point inPlanePosition, RandomEngine::random_airport_info_struct &inout_random_airport_info_struct, const bool in_flag_we_have_target_above_water)
{
  ...
  na_briefer.fpln_xml_target_leg_node = x_briefer_node.deepCopy ();
...
RandomEngine::gen_post_briefer_desc (std::map<int, NavAidInfo> &in_osm_na_targets, const bool flag_has_wet_target)
{
  ...
  Utils::xml_add_cdata (in_osm_na_targets[0].fpln_xml_target_leg_node, briefer_desc);
}
...
RandomEngine::gen_leg_start_messages (int &seq, NavAidInfo &inout_target_na, IXMLNode &inout_xml_messages)
{
  ...
  const std::string msg_text = fmt::format("Fly to: {}.", inout_target_na.get_loc_desc());
    ...
    !msg_node.isEmpty ())
  {
    ...
    Utils::xml_add_cdata (msg_mix_text_node, msg_text);
...
void
RandomEngine::gen_3d_object_set (const NavAidInfo &inout_target_na, IXMLNode &inout_leg_node, IXMLNode &in_template_node, IXMLNode &inout_x3DObjTemplate, double &in_expected_slope_at_target_location_d)
{
  ...
  // read <desc> node
  IXMLNode xml_desc_ptr = inout_leg_node.getChildNode (mxconst::get_ELEMENT_DESC ().c_str ());
  ...
  const auto lmbda_get_source_of_3d_set =[] (std::unordered_map<enum_set_3d_source, set_3d_strct> & map_3d_set_attributes)
  {
  ...
  const enum_set_3d_source picked_3d_set_source = lmbda_get_source_of_3d_set (map_3d_set_attributes);
  if (picked_3d_set_source != enum_set_3d_source::none)
  {
    ...
    // check slope and use slope set if it has value.
    ...
    ///////////////////////////////////////
    const auto lmbda_add_all_display_object_xxx_elements = [&] (const std::string& func_name, const IXMLNode &inParentNode, IXMLNode &inoutTargetNode)
    {
      const int nDisplayObjects = inParentNode.nChildNode ();
      for (int i1 = 0; i1 < nDisplayObjects; ++i1)
      {
        ...
        #ifndef RELEASE
        Log::logMsgThread (fmt::format ("[{}]Adding 3D display_objects from: {}:{}", func_name, tag, Utils::readAttrib (cNode, mxconst::get_ATTRIB_NAME (), "") ) );
        #endif

        if (std::string err
          ; RandomEngine::parse_display_object_element (inoutTargetNode, cNode, in_template_node, inout_x3DObjTemplate, in_expected_slope_at_target_location_d, err)) // v25.06.1 extended function signature // v3.0.219.1 handle <display_object> options like: optional, random_water or limit_to_terrain_slope
        {
          if (tag == mxconst::get_ELEMENT_DISPLAY_OBJECT_NEAR_PLANE ())
          {
            //  Force replace_lat / replace_long dummy values to be on the safe side
...
RandomEngine::gen_3d_hint_objects_for_land_and_hover (const NavAidInfo &inout_target_na, IXMLNode &inout_leg_node, const NavAidInfo *next_navaid_ptr)
{
  ...
  const int landing_radius_mt = (mxconst::get_FL_TEMPLATE_VAL_LAND_HOVER () == inout_target_na.fpln_osm_wp_type) ? LANDING_RADIUS_FOR_LAND_HOVER_MT : LANDING_RADIUS_FOR_LAND_ONLY_MT;
  const int how_many_3d_objects_to_display  = (mxconst::get_FL_TEMPLATE_VAL_LAND_HOVER () == inout_target_na.fpln_osm_wp_type) ? 24 : 4;
  ...
  if (mxconst::get_FL_TEMPLATE_VAL_LAND_HOVER () == inout_target_na.fpln_osm_wp_type )
  {
...
RandomEngine::gen_targets_using_osm_queries_from_a_thread (missionx::base_thread::thread_state *inoutThreadState, const IXMLNode &in_root_node, missionx::structs::strct_osm_query &inout_osm_query, random_airport_info_struct &inout_shared_navaid)
{
  ...
  while (!flag_all_osm_queries_are_done && seq < max_loops_i && !inoutThreadState->flagAbortThread)
  {
    ...
    if (!inout_osm_query.xml_target_nd_node.isEmpty () && !inout_osm_query.xml_target_way_element.isEmpty ())
    {
      ...
      target_navaid.fpln_osm_wp_type    = Utils::readAttrib (inout_osm_query.xml_query_node_to_search_a_new_target, "wp_type", "");
      target_navaid.fpln_xml_osm_q_node = inout_osm_query.xml_query_node_to_search_a_new_target.deepCopy ();
      target_navaid.fpln_xml_way_node   = inout_osm_query.xml_target_way_element.deepCopy ();

      // v25.06.1 clear the <header> node. This is the root node of current <q> sub-node
      ...
      // search nearest navaid if not hover_land
      if (target_navaid.fpln_osm_wp_type == mxconst::get_FL_TEMPLATE_VAL_LAND ())
      {
        ...
        !next_tag.empty ())
      {
        ...
        for (size_t counter = 0; const auto &v_index : vec_shuffled_next_tag)
        {
          ...
          if (inout_osm_query.xml_q_tags_header_node.isEmpty () && counter < vec_shuffled_next_tag.size ())
            continue;
          else if (inout_osm_query.xml_q_tags_header_node.isEmpty () )
          {
            flag_all_osm_queries_are_done = true;
            seq = max_loops_i;
            break;
...
RandomEngine::prepare_medevac_surprise_me (IXMLNode &inRootTemplate, const IXMLNode &inoutMetaNode, const missionx::Point& in_plane_location)
{
  ...
  for (const auto &randomNumber : vec_shuffle_subject_queries)
  {
    ...
    // ----------------------
    osm_na_targets = gen_targets_using_osm_queries_from_a_thread (&RandomEngine::threadState, osm_gen_xml_root_node, osm_query, this->shared_navaid_info );
  ...
  for (auto &target_navaid : osm_na_targets | std::views::values)
  {
    ...
    // store slope at target location
  ...
  if (!osm_na_targets.empty())
  {
    ...
    for (auto &[indx, target_navaid] : osm_na_targets)
    {
      ...
      const std::list<missionx::structs::strct_node_attribute_key_value> lsAttrib_trig_land  =
        ...
        { "radius", "length_mt", (target_navaid.fpln_osm_wp_type == mxconst::get_FL_TEMPLATE_VAL_LAND_HOVER())? mxconst::DEFAULT_LAND_OR_INV_RADIUS_MT.data() : mxconst::DEFAULT_HOVER_RADIUS_MT.data () }
        };
      ...
      const auto inv_radius = (target_navaid.fpln_osm_wp_type == mxconst::get_FL_TEMPLATE_VAL_LAND_HOVER())? mxconst::DEFAULT_LAND_OR_INV_RADIUS_MT.data () : mxconst::DEFAULT_HOVER_RADIUS_MT.data ();
...
order by RANDOM() limit 1
  ...
  switch (missionx::data_manager::ui_oilrig_globe_part_i)
  {
    ...
    {
      // Longitude: between -180..0 or 0..180
      constexpr double LON_BOUND_RADIUS = 40.0;
      constexpr double LAT_BOUND_RADIUS = 25.0;
  ...
  const int nChildLegs = inRootTemplate.nChildNode (mxconst::get_ELEMENT_LEG ().c_str ());
                                                              ...
                                                              const double location_maxDistance_d)
{
...
#ifdef IBM
  outNewNavInfo = this->get_random_airport_from_db (this->shared_navaid_info.p, this->shared_navaid_info.inMinDistance_nm, this->shared_navaid_info.inMaxDistance_nm, this->shared_navaid_info.inExcludeAngle, inProperties); // v3.0.255.3 test integration
#else
  const NavAidInfo nav = this->get_random_airport_from_db (this->shared_navaid_info.p, this->shared_navaid_info.inMinDistance_nm, this->shared_navaid_info.inMaxDistance_nm, this->shared_navaid_info.inExcludeAngle, inProperties); // v3.0.255.3 test integration
  outNewNavInfo  = nav;
#endif
  ...
  if ((outNewNavInfo.lat == 0.0f || outNewNavInfo.lon == 0.0f) && this->mapNavAidsFromMainThread.empty () && location_value_d > 0.0)
  {
    ...
    outNewNavInfo = this->get_random_airport_from_db (this->shared_navaid_info.p, this->shared_navaid_info.inMinDistance_nm, this->shared_navaid_info.inMaxDistance_nm, this->shared_navaid_info.inExcludeAngle, inProperties); // v3.0.255.3 test integration
#else
    NavAidInfo nav = this->get_random_airport_from_db (this->shared_navaid_info.p, this->shared_navaid_info.inMinDistance_nm, this->shared_navaid_info.inMaxDistance_nm, this->shared_navaid_info.inExcludeAngle, inProperties); // v3.0.255.3 test integration
    outNewNavInfo  = nav;
#endif
