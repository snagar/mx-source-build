...
XPluginStart (char *outName, char *outSig, char *outDesc)
{
  ...
  if (missionx::Mission::uiImGuiBriefer == nullptr)
  {
    ...
    // Position widget at the center of the screen
  ...
  XPLMRegisterKeySniffer (missionx::MyKeySniffer,
                          /* handling function callback. */
                          1,
                          /* Receive input before plugin windows. */
  ...
  if (missionx::data_manager::init_xp_airport_db ()) // v3.0.255.3 The airport database is a key part in the Random Mission builder. We need to establish validity when plugin starts
  {
...
drawCallback_missionx (XPLMDrawingPhase inPhase, int inIsBefore, void *inRefcon)
{
  ...
  auto start = chrono::steady_clock::now ();
  #endif
  // for each draw callback phase we should call a different draw function
...
setSharedDataRef (std::string inDataName, XPLMDataTypeID inDataType, XPLMDataChanged_f inNotificationFunc, double inValue)
{
  ...
  if (status)
  {
    ...
    if (dref.flag_paramReadyToBeUsed)
    {
      ...
      Log::logMsg (fmt::format ("missionx: Added shared dataref: {}\t {}", inDataName, dref.to_string ()));
      #endif
    }
    else
    {
...
MyKeySniffer (char inChar, XPLMKeyFlags inFlags, char inVirtualKey, void *inRefcon)
{
  ...
  if (inVirtualKey == XPLM_VK_ESCAPE)
  {
    ...
    } // end if a window is visible and not in VR
