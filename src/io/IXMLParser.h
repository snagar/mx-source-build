...
#include <stdlib.h>
#include <string> // added by saar // for std::string
...
#include <string.h> // added by saar // for _strnicmp and _stricmp
...
#define XML_NO_WIDE_CHAR // saar missionx
...
public:
   ...
   */
  virtual void setNormalBufferSize([[maybe_unused]] int nbyte) {} // saar missionx
  ...
  virtual void setSource([[maybe_unused]]IXMLReader* source) {} ///< do not use // saar missionx
...
public:
  ...
  char ignoreUnknownEntities, ignoreEmptyTag;
private:
  // These are functions and structures used internally by the IXMLPullParser class (don't bother about them):
  int maxAttribute;
...
public:
  ...
  ITCXMLNode openFileHelper(IXMLCStr filename, IXMLCStr tag = NULL, std::string* outErr = nullptr); // modified by Saar for missionx
...
public:
  ...
  char isClosedNode() const;                                      ///< is this node directly closed? such as "<A/>"
...
protected:
  ...
  typedef struct IXMLNodeData // to allow shallow copy and "intelligent/smart" pointers (automatic delete):
  {
    ...
    char                    isClosedNode;           // Whether node is directly closed '<A/>'
    ...
    inline void init(struct IXMLNodeData* pParent, char isDeclaration, char _isClosedNode);
  } IXMLNodeData;
  IXMLNodeData* d;
...
public:
  ...
  IXMLStr toJSONUnSafe(IXMLStr dest, IXMLCStr source);
  IXMLStr toXMLUnSafe(IXMLStr dest, IXMLCStr source); ///< deprecated: use "toXML" instead
  int lengthJSONString(IXMLCStr source);
 ...
 * @{ */
 ...
 //class IXMLDLLENTRY IJSONRenderer
 //{
 //public:
 //	IJSONRenderer();
 //    IJSONRenderer(IXMLCStr tagNameForArrays);
 //    ~IJSONRenderer();
 //    static IXMLCStr getErrorMessage(IXMLErrorInfo error); ///< this gives you a user-friendly explanation of the parsing error
 //
 //     /** @defgroup CreateString Create an JSON string starting from the current IXMLNode.
 //     * @ingroup CreatingXML
 ...
 //     * @{ */
 //    IXMLCStr getString(ITCXMLNode x, int *pnSize=NULL); ///< Create an XML string starting from the current ITCXMLNode.
 //	void clear();
 //	int sz;
 //
 //private:
 //	IXMLCStr tagNameForArrays;
 //	IXMLStr buf;
 //	int maxStrLen;
 //	ToIXMLStringTool txAtt,txTxt;
 //	//int getStringR(ITCXMLNode::IXMLNodeData *pEntry, IXMLStr lpcMarker);
 //};
