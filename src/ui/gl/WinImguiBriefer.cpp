...
// Define our own window title
  ...
  if (!this->vecExternalCategories.empty () && this->vecExternalCategories.size() > static_cast<size_t>(1)) // At least Two items should be in the sub-category.
  {
    ...
    this->mapMissionCategories[static_cast<int> (missionx::mx_mission_type::cargo)] = data_manager::strct_ui_share_data.cargo_arr;
  }
...
WinImguiBriefer::draw_dynamic_mission_creation_screen ()
{
  ...
  if (this->strct_user_create_layer.layer_state == missionx::mx_layer_state_enum::success_can_draw)
  {
    ...
    ImGui::BeginChild ("draw_user_create_mission_layer_01", ImVec2 (0.0f, win_size_vec2.y * 0.77f), ImGuiChildFlags_Borders);
    {
      ...
      const bool bPickedOilRigMission            = this->strct_user_create_layer.iRadioMissionTypePicked == static_cast<int> (missionx::mx_mission_type::oil_rig);
      const bool bPickedMedevacMission           = this->strct_user_create_layer.iRadioMissionTypePicked == static_cast<int> (mx_mission_type::medevac);
