...
private:
  int            iFontQueue{ 0 };
  // ImPlotContext* mImPlotContext = ImPlot::CreateContext();
...
public:
  ...
  void mxUiReleaseLastFont(int inHowManyCycles = 1); // pop out Fonts we pushed, default is only the last one but you can release more than one. Updates iFontQueue.

  bool mxStartUiDisableState(bool in_true_exp_to_disable); // v24.02.6 true means not disable
  void mxEndUiDisableState(bool in_true_exp_to_disable);   // v24.02.6 true means not disable
